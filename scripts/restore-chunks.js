/**
 * Zero-cost restore of all multi-journey chunks from scripts/chunks-backup.json.
 * No Claude calls — just re-writes every cached entry to Upstash. Use this after
 * an accidental cache loss instead of regenerating (which costs API spend).
 *
 * Encoding matches api/journey-chunk.js cacheGet (double-encoded).
 * NEVER use FLUSHDB. To clear specific keys, delete them surgically.
 *
 * Usage:  node scripts/restore-chunks.js
...
if (!fs.existsSync(BACKUP_PATH)) {
  console.error(`No backup found at ${BACKUP_PATH}. Run: node scripts/generate-chunks.js first.`);
  process.exit(1);
}
...
async function main() {
  ...
  console.log(`\n♻️  Restoring ${entries.length} chunks from backup (saved ${snap.saved_at})...\n`);
